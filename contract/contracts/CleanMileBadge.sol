...
contract CleanMileBadge is ERC1155, ICleanMileBadge ,Ownable {
    ...
    event TransferBatch(address indexed sender, address[] recipients, uint256 tokenId, uint256 amount);

    constructor() ERC1155("") {}

    // 뱃지를 발급하는 함수입니다.
    function mintBadge(address account, BadgeType badgeType, uint256 amount, string memory _uri) external onlyOwner {
    ...
    function badgeBalance(address account, uint256 tokenId) public view returns (uint256) {
    ...
    function transferBadge(address from, address to, uint256 tokenId, uint256 amount) public {
        require(from == msg.sender || isApprovedForAll(from, msg.sender) || approvalForToken(msg.sender, tokenId), "Caller is not approved");
    ...
    function transferBadges(address[] calldata recipients, uint256 tokenId, uint256 amount) public {
        ...
        uint256 currentBadageScore = badgeScore[uint256(_tokenToBadgeType[tokenId])];
    ...
    function approvalForToken(address account, uint256 tokenId) public view returns (bool) {
