...
describe('client/usersControllerTest', () => {
  ...
  try {
    ...
    const userData = new UserModel({
        ...
        user_type: "user",
        hashed_pw: "$2b$10$IspODcbUqPxCaVqZMsBkmOUZxktY1wsnVfW2V0iMQmagmdR0DI1da",
        nickname: "Test",
...
it('pw를 제거한 사용자 정보 조회', async () => {
  try {
    ...
    // 사용자 정보가 pw를 제외한 형태인지 검증합니다.
    const userData = result.data;
    expect(userData).to.not.have.property('hashed_pw');
  } catch (err) {
    throw err;
  }
});
...
it('General, Review Posts List 조회', async () => {
 ...
 it('getProfile 함수 테스트', async () => {
    ...
    // expect(result.data).to.have.property('dnft');
    // expect(result.data).to.have.property('badges');
